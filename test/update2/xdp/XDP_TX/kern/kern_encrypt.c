...
int xdp_sm4_encrypt(struct xdp_md *ctx)
{
    ...
    u8 *encrypt_ptr = payload + 12;
    u32 encrypt_len = payload_len > 12 ? payload_len - 12 : 0;
...
#pragma unroll
    for (int i = 0; i < 100; i++)
    {
        ...
        u8 block_buf[16] = {0};
        if ((void *)(encrypt_ptr + i + i * 16 + 16) > data_end)
            break;
        __builtin_memcpy(block_buf, encrypt_ptr + i * 16, 16);
...
#pragma unroll
        for (int j = 0; j < 4; j++)
        {
            ...
            __builtin_memcpy(encrypt_ptr + i * 16 + j * 4, &tmp, 4);
        }
    }
