...
void print_test_summary(int total_packets, int lost_packets, double avg_delay, double min_delay, double max_delay, double p95_delay, double p99_delay, double stddev, double jitter, double duration) {
...
int main() {
    ...
    while (count < MAX_PACKET_NUM) {
        ...
        if (current_time - last_output_time >= 1000000000ULL) {  // 每秒刷新一次
            ...
            // 计算标准差
            double mean_square = sum_squared_latency / latency_count;
            double variance = mean_square - average_latency_us * average_latency_us;
            double stddev = sqrt(variance);

            // 计算抖动
            double jitter = latencies_us[latency_count - 1] - latencies_us[0];

            // 输出实时统计
            print_test_summary(count, loss_count, average_latency_us, min_latency / 1000.0, max_latency / 1000.0, p95_latency_us, p99_latency_us, stddev, jitter, duration_sec);
        }
    }
