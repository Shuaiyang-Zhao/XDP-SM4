...
/*
 * XDP 入口函数：
 * 解析以太网/IP/UDP头部，如果为 UDP 包且负载大于等于 16 字节，
 * 则用固定的密钥对 UDP 负载的前 16 字节进行 SM4 加密。
 */
SEC("xdp")
int xdp_sm4_encrypt(struct xdp_md *ctx)
{
    ...
    if (pad)
    {
        ...
        // 更新 UDP 长度字段：UDP header 长度 = 8 + payload_len
        udp->len = bpf_htons(sizeof(*udp) + payload_len);

        // 更新 IP 总长度字段：IP header 长度 = ip->ihl*4 + UDP header 长度
...
#pragma unroll
    for (int i = 0; i < 32; i++)
    {
        ...
        // 将 block_buf 按 4 字节一组转换为 u32 数组（网络序转主机序）
        u32 block[4] = {0};
...
#pragma unroll
    ...
    ip->daddr = bpf_htonl(0xC0A83A83); // 192.168.58.131
    bpf_printk("修改目标IP为: %pI4", &ip->daddr); // 调试输出

    struct bpf_fib_lookup fib_params = {
        .family = AF_INET,
        .ipv4_dst = ip->daddr, // 使用修改后的目标IP
    ...
    if (rc == BPF_FIB_LKUP_RET_SUCCESS) {
        ...
        // 添加详细的错误处理
        bpf_printk("FIB查找失败: 错误码=%d 请检查路由和ARP表", rc);
        
        // 回退处理：让内核栈处理（需要开启ip_forward）
        ip->check = 0;
        ip->check = ip_checksum(ip);
        udp->check = 0;
        udp->check = udp_checksum(ip, udp, data_end);
