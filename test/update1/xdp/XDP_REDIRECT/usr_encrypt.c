#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <arpa/inet.h>
#include <netinet/ip.h>
#include <netinet/udp.h>
#include <sys/socket.h>
#include <net/ethernet.h>

#define SM4_BLOCK_SIZE 16
#define BUF_SIZE 2048

// 引入SM4加密相关的代码（包括S-box、扩展常数等）
#define u8 unsigned char
#define u32 unsigned int
#define u16 unsigned short

#define SM4_BLOCK_SIZE 16
// S盒（放在只读段）
static const u8 Sbox[256] __attribute__((section(".rodata"))) = {
    };

// 密钥扩展常数
static const u32 FK[4] __attribute__((section(".rodata"))) = {
    0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc};

// 固定参数 CK
static const u32 CK[32] __attribute__((section(".rodata"))) = {
    0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
    0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
    0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
    0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
    0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
    0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
    0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
    0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279};

static __always_inline u32 loopLeft(u32 a, short length)
{
    return (a << length) | (a >> (32 - length));
}

static __always_inline u32 functionB(u32 b)
{
    u8 a0 = (u8)(b >> 24);
    u8 a1 = (u8)(b >> 16);
    u8 a2 = (u8)(b >> 8);
    u8 a3 = (u8)(b);
    return ((u32)Sbox[a0] << 24) | ((u32)Sbox[a1] << 16) | ((u32)Sbox[a2] << 8) | Sbox[a3];
}

static __always_inline u32 functionL1(u32 a)
{
    return a ^ loopLeft(a, 2) ^ loopLeft(a, 10) ^ loopLeft(a, 18) ^ loopLeft(a, 24);
}

static __always_inline u32 functionL2(u32 a)
{
    return a ^ loopLeft(a, 13) ^ loopLeft(a, 23);
}

static __always_inline u32 functionT(u32 a, short mode)
{
    return mode == 1 ? functionL1(functionB(a)) : functionL2(functionB(a));
}

static __always_inline void extendFirst(u32 MK[4], u32 K[4])
{
#pragma unroll
    for (int i = 0; i < 4; i++)
    {
        K[i] = MK[i] ^ FK[i];
    }
}

static __always_inline void extendSecond(u32 RK[32], u32 K[4])
{
#pragma unroll
    for (int i = 0; i < 32; i++)
    {
        u32 temp = K[i % 4] ^ functionT(K[(i + 1) % 4] ^ K[(i + 2) % 4] ^ K[(i + 3) % 4] ^ CK[i], 2);
        K[(i + 4) % 4] = temp;
        RK[i] = temp;
    }
}

static __always_inline void getRK(u32 MK[4], u32 K[4], u32 RK[32])
{
    extendFirst(MK, K);
    extendSecond(RK, K);
}

static __always_inline void iterate32(u32 X[8], u32 RK[32])
{
#pragma unroll
    for (int i = 0; i < 32; i++)
    {
        u32 temp = X[i % 4] ^ functionT(X[(i + 1) % 4] ^ X[(i + 2) % 4] ^ X[(i + 3) % 4] ^ RK[i], 1);
        X[(i + 4) % 4] = temp;
    }
}

static __always_inline void reverse(u32 X[8], u32 Y[4])
{
#pragma unroll
    for (int i = 0; i < 4; i++)
    {
        Y[i] = X[3 - i];
    }
}

static __always_inline void encryptSM4(u32 X[4], u32 RK[32], u32 Y[4])
{
    iterate32(X, RK);
    reverse(X, Y);
}

static __always_inline u16 ip_checksum(struct iphdr *ip)
{
    u32 sum = 0;
    u16 *ptr = (u16 *)ip;
    ip->check = 0; // 必须先将校验和字段清零

#pragma unroll
    for (int i = 0; i < sizeof(struct iphdr) / 2; i++)
    {
        sum += ptr[i];
    }

    // 处理进位
    sum = (sum & 0xffff) + (sum >> 16);
    sum += (sum >> 16);
    return ~sum;
}

/* 计算 UDP 伪首部校验和 */
static __always_inline u32 udp_pseudo_checksum(struct iphdr *ip, struct udphdr *udp, u16 len)
{
    u32 sum = 0;

    // 伪首部：源IP + 目的IP + 协议 + UDP长度
    sum += (ip->saddr >> 16) & 0xffff;
    sum += ip->saddr & 0xffff;
    sum += (ip->daddr >> 16) & 0xffff;
    sum += ip->daddr & 0xffff;
    sum += bpf_htons(IPPROTO_UDP);
    sum += udp->len;

    // UDP 头部
    sum += bpf_ntohs(udp->source);
    sum += bpf_ntohs(udp->dest);
    sum += bpf_ntohs(udp->len);

    // 计算负载部分（需要处理奇数长度）
    u8 *payload = (u8 *)udp + sizeof(*udp);
    u16 payload_words = len / 2;

#pragma unroll
    for (int i = 0; i < payload_words; i++)
    {
        u16 word;
        bpf_probe_read_kernel(&word, 2, payload + i * 2);
        sum += bpf_ntohs(word);
    }

    // 处理最后一个字节（奇数长度时）
    if (len % 2)
    {
        u16 final = 0;
        bpf_probe_read_kernel(&final, 1, payload + len - 1);
        sum += bpf_ntohs(final << 8);
    }

    return sum;
}

/* 计算完整的 UDP 校验和 */
static __always_inline u16 udp_checksum(struct iphdr *ip, struct udphdr *udp, u16 len)
{
    u32 sum = udp_pseudo_checksum(ip, udp, len);

    // 折叠进位并取反
    sum = (sum & 0xffff) + (sum >> 16);
    sum += (sum >> 16);
    return (u16)~sum;
}

// 使用您提供的SM4加密算法相关函数来加密UDP数据

// 以下是原始套接字接收UDP包并进行加密的代码
int main() {
    int sock;
    struct sockaddr_in server, client;
    unsigned char buffer[BUF_SIZE];
    ssize_t packet_size;

    // 创建原始套接字
    sock = socket(AF_INET, SOCK_RAW, IPPROTO_UDP);
    if (sock < 0) {
        perror("Socket creation failed");
        exit(EXIT_FAILURE);
    }

    // 绑定套接字
    memset(&server, 0, sizeof(server));
    server.sin_family = AF_INET;
    server.sin_addr.s_addr = inet_addr("192.168.58.128"); // 监听所有接口
    server.sin_port = htons(0); // 任意端口
    if (bind(sock, (struct sockaddr *)&server, sizeof(server)) < 0) {
        perror("Bind failed");
        close(sock);
        exit(EXIT_FAILURE);
    }

    while (1) {
        packet_size = recvfrom(sock, buffer, BUF_SIZE, 0, NULL, NULL);
        if (packet_size < 0) {
            perror("Packet reception failed");
            continue;
        }

        struct iphdr *ip = (struct iphdr *)(buffer + sizeof(struct ethhdr));
        struct udphdr *udp = (struct udphdr *)(buffer + sizeof(struct ethhdr) + ip->ihl * 4);

        if (ip->protocol == IPPROTO_UDP) {
            unsigned char *payload = (unsigned char *)(buffer + sizeof(struct ethhdr) + ip->ihl * 4 + sizeof(struct udphdr));
            int payload_len = ntohs(udp->len) - sizeof(struct udphdr);

            // 加密数据包
            if (payload_len >= SM4_BLOCK_SIZE) {
                // 使用您提供的SM4加密算法
                u32 MK[4] = {0x01234567, 0x89abcdef, 0xfedcba98, 0x76543210};
                u32 K[4] = {0};
                u32 RK[32] = {0};
                getRK(MK, K, RK);

                // 分块加密每个块
                for (int i = 0; i < payload_len / SM4_BLOCK_SIZE; i++) {
                    unsigned char block_buf[SM4_BLOCK_SIZE] = {0};
                    memcpy(block_buf, payload + i * SM4_BLOCK_SIZE, SM4_BLOCK_SIZE);

                    u32 block[4] = {0};
                    for (int j = 0; j < 4; j++) {
                        memcpy(&block[j], block_buf + j * 4, 4);
                        block[j] = ntohl(block[j]);
                    }

                    u32 encrypted[4] = {0};
                    encryptSM4(block, RK, encrypted);

                    for (int j = 0; j < 4; j++) {
                        u32 tmp = htonl(encrypted[j]);
                        memcpy(payload + i * SM4_BLOCK_SIZE + j * 4, &tmp, 4);
                    }
                }
            }

            // 更新UDP校验和
            udp->check = 0;
            udp->check = udp_checksum(ip, udp, payload_len);

            // 发送加密后的数据包
            if (sendto(sock, buffer, packet_size, 0, (struct sockaddr *)&client, sizeof(client)) < 0) {
                perror("Send failed");
            }
        }
    }

    close(sock);
    return 0;
}
