...
int main() {
    ...
    // 2. 打开 IP_HDRINCL，以便 sendto 时自带 IP 头
    int one = 1;
    ...
    server.sin_addr.s_addr = inet_addr("192.168.58.128");
    ...
    if (ip->protocol == IPPROTO_UDP) {
        ...
        // 直接用包头中原始的目的 IP 和端口（网络字节序）
        forward_addr.sin_addr.s_addr = ip->daddr;
        forward_addr.sin_port        = udp->dest;

        if (sendto(sock, buffer, packet_size, 0,
                   (struct sockaddr *)&forward_addr, sizeof(forward_addr)) < 0) {
            perror("sendto forward");
        } else {
            printf("已加密并转发 %zd 字节 到 %s:%u\n",
                   packet_size,
                   inet_ntoa(forward_addr.sin_addr),
                   ntohs(forward_addr.sin_port));
        }
    }
