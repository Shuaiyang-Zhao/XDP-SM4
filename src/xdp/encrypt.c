...
int xdp_sm4_encrypt(struct xdp_md *ctx)
{
    ...
    // 只处理特定源地址和端口的数据包
    if (ip->saddr != bpf_htonl(0xc0a83a84) || udp->source != bpf_htons(1235))
    {
    ...
    if (pad)
    {
        ...
        ip->tot_len = bpf_htons(bpf_ntohs(ip->tot_len) + pad);
    }

    bpf_printk("adjusted payload_length: %d", payload_len);
    ...
    if (!sm4key)
    {
        bpf_printk("No encryption key found in map!");
        return XDP_PASS;
    }
