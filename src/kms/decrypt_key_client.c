...
int ensure_bpf_dir_exists() {
    ...
    // 确保目录存在
    const char *map_dir = "/sys/fs/bpf/sm4_key_decrypt";
...
void show_maps_status() {
    ...
    printf("\n3. Pinned map at /sys/fs/bpf/sm4_key_decrypt/sm4_key_map:\n");
    system("bpftool map dump pinned /sys/fs/bpf/sm4_key_decrypt/sm4_key_map");
}

int main(int argc, char **argv) {
    if (argc < 3) {
        printf("Usage: %s <kms_server_ip> <bpf_map_path> [poll_interval] [port]\n", argv[0]);
        printf("Example: %s 192.168.58.132 /sys/fs/bpf/sm4_key_decrypt/sm4_key_map 60 9000\n", argv[0]);
        return 1;
    }
